...
class AttackComboConfig {
	...
	public function draw():Void {
		...
		if (HudChrome.beginPanel("Attack Combo Tracker##sf_actcfg", open, "Attack Combo Tracker")) {
			...
			UiLayout.propertyGrid("##act_window_props", function() {
				UiLayout.propertyRow("Size", function() {
					UiLayout.inlinePair(
						"##act_size",
						function(_:Single) {
							if (ImGui.sliderFloat("Width##act_w", width, MIN_W, MAX_W, "%.0f px")) {
								sizeDirty = true;
								SettingsStore.markDirty();
							}
						},
						function(_:Single) {
							if (ImGui.sliderFloat("Height##act_h", height, MIN_H, MAX_H, "%.0f px")) {
								sizeDirty = true;
								SettingsStore.markDirty();
							}
						}
	...
	public function drawAppearanceSettings():Void {
		UiLayout.propertyGrid("##act_appear_props", function() {
			UiLayout.propertyRow("Type", function() {
				if (intCombo("##act_type", "at", comboType, TYPE_ITEMS)) SettingsStore.markDirty();
			});
			if (comboType.get() == TYPE_PIP) {
				UiLayout.propertyRow("Display", function() {
					if (intCombo("##act_disp", "adm", displayMode, DISPLAY_MODE_ITEMS)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Colors", function() {
					if (intCombo("##act_col", "acs", colorScheme, COLOR_SCHEME_ITEMS)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Options", function() {
					var avail:Single = ImGui.getContentRegionAvail().x;
					var colW:Single = avail / 3;
					if (ImGui.checkbox("Vertical##act_vert", vertical)) SettingsStore.markDirty();
					ImGui.sameLine(colW);
					if (ImGui.checkbox("Glow FX##act_glow", showGlow)) SettingsStore.markDirty();
					ImGui.sameLine(colW * 2);
					if (ImGui.checkbox("Step #s##act_labels", showLabels)) SettingsStore.markDirty();
				});
			} else {
				UiLayout.propertyRow("Style", function() {
					if (intCombo("##act_custom_style", "acst", customStyle, CUSTOM_STYLE_ITEMS)) SettingsStore.markDirty();
				});
			}
		});
