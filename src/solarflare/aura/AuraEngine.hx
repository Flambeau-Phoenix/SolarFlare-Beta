...
class AuraEngine {
	...
	static function eval(a:AuraDef, now:Float):Void {
		...
		var prog:Float = 1;
		var stacks = 1;
		// Seconds remaining published by triggers that know one; NaN leaves the aura timerless.
		var sourceLeft = Math.NaN;
		var t = a.trigger;
		if (t == "resource") {
	...
	static function logHold(a:AuraDef, now:Float):{hit:Bool, progress:Float, left:Float} {
		var want = a.skillId != null ? StringTools.trim(a.skillId) : "";
		if (want.length < 2)
			return {hit: false, progress: 0, left: Math.NaN};
		...
		if (left < 0)
			left = 0;
		return {hit: true, progress: hold > 0.05 ? clamp01(left / hold) : 1, left: left};
	}
	...
	static function toObjUnsafe(a:AuraDef):Dynamic {
		...
		var obj:Dynamic = {
			...
			countdownScale: floatRef(a.countdownScale, 1),
