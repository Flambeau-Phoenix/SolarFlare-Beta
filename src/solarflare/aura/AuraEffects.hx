...
class AuraEffects {
	...
	/** True when the Glow tile / a KIND_ICON effect asks for glow, even if live hit is false. */
	public static function hasIconGlow(a:AuraDef):Bool {
		if (a == null || a.effects == null)
			return false;
		for (e in a.effects) {
			if (e != null && e.enabled.get() && e.kind == AuraEffect.KIND_ICON && e.glow.get())
				return true;
		}
		return false;
	}

	/** Persist Glow through a KIND_ICON effect so AuraEffects.apply cannot wipe the tile. */
	public static function setIconGlow(a:AuraDef, on:Bool):Void {
		if (a == null)
			return;
		AuraEffects.ensure(a);
		var found:AuraEffect = null;
		for (e in a.effects) {
			if (e != null && e.kind == AuraEffect.KIND_ICON) {
				found = e;
				break;
			}
		}
		if (!on) {
			if (found != null)
				found.glow.set(false);
			a.iconGlow = false;
			return;
		}
		if (found == null) {
			found = new AuraEffect("glow", AuraEffect.KIND_ICON, windowWhen(a));
			a.effects.push(found);
		}
		found.enabled.set(true);
		found.glow.set(true);
	}

	static function windowWhen(a:AuraDef):String {
		if (a.effects != null) {
			for (e in a.effects) {
				if (e != null && e.kind == AuraEffect.KIND_WINDOW)
					return e.when;
			}
		}
		return AuraEffect.WHEN_WHILE_TRUE;
	}

	static function takeIconGlow(a:AuraDef):AuraEffect {
		if (a == null || a.effects == null)
			return null;
		for (e in a.effects) {
			if (e != null && e.kind == AuraEffect.KIND_ICON && e.glow.get())
				return e;
		}
		return null;
	}
