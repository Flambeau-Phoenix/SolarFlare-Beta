...
class GeauxConfig {
	...
	public function drawHotkeyAssignment(i:Int):Void {
		...
		if (i < 0 || i >= visibleCount())
			return;
		ImGui.textWrapped("Display label for this slot's assigned keyboard/controller binding. This does not capture or replace Farever input.");
		ImGui.setNextItemWidth(150);
		if (ImGui.inputText("Key assignment##gb_hk" + i, hotkeyBytes[i].getData(), HOTKEY_BUF)) {
		...
		ImGui.sameLine();
		if (ImGui.smallButton("Clear##gb_hk_clear" + i)) {
			slotHotkeys[i] = "";
			writeHotkeyBuf(i, "");
			SettingsStore.markDirty();
