...
class VectorEffect {
	...
	public function isDead():Bool {
		...
		for (p in particles) if (p.life > 0) return false;
		return true;
	}

	public function draw(dl:Dynamic, originX:Float = 0, originY:Float = 0, scale:Float = 1):Void {
		if (scale <= 0) scale = 1;
		var currentColor = lerpColor(startColor, endColor, progress);
		var currentRadius = (startRadius + (endRadius - startRadius) * progress) * scale;
		var drawX = originX + x * scale;
		var drawY = originY + y * scale;

		switch (type) {
			case EffectType.Pulse:
				drawPulse(dl, drawX, drawY, currentRadius, currentColor, scale);
			case EffectType.Ring:
				drawRing(dl, drawX, drawY, currentRadius, currentColor, scale);
			case EffectType.Spark:
				drawSpark(dl, drawX, drawY, currentRadius, currentColor, scale);
			case EffectType.Particles:
				drawParticles(dl, originX, originY, scale);
			case EffectType.Beam:
				drawBeam(dl, drawX, drawY, currentRadius, currentColor, scale);
		}
	}

	function drawPulse(dl:Dynamic, drawX:Float, drawY:Float, radius:Float, color:Int, scale:Float):Void {
		var a = (color >> 24) & 0xFF;
		...
		for (i in 0...3) {
			...
			ImGui.ImDrawList_AddCircle(dl, ImGui.vec2(drawX, drawY), r, col, segments, thickness * scale);
		}
	}

	function drawRing(dl:Dynamic, drawX:Float, drawY:Float, radius:Float, color:Int, scale:Float):Void {
		ImGui.ImDrawList_AddCircle(dl, ImGui.vec2(drawX, drawY), radius, color, segments, thickness * scale);
	}

	function drawSpark(dl:Dynamic, drawX:Float, drawY:Float, radius:Float, color:Int, scale:Float):Void {
		for (i in 0...8) {
			var angle = (i / 8.0) * Math.PI * 2.0 + progress * 2.0;
			var length = radius * (0.3 + 0.7 * (1.0 - progress));
			var x1 = drawX + Math.cos(angle) * radius * 0.1;
			var y1 = drawY + Math.sin(angle) * radius * 0.1;
			var x2 = drawX + Math.cos(angle) * length;
			var y2 = drawY + Math.sin(angle) * length;
			var alpha = Std.int(255 * (1.0 - progress));
			var col = (color & 0x00FFFFFF) | (alpha << 24);
			ImGui.ImDrawList_AddLine(dl, ImGui.vec2(x1, y1), ImGui.vec2(x2, y2), col, thickness * scale);
		}
	}

	function drawParticles(dl:Dynamic, originX:Float, originY:Float, scale:Float):Void {
		for (p in particles) {
			if (p.life <= 0) continue;
			var alpha = Std.int(((startColor >> 24) & 0xFF) * (p.life / p.maxLife));
			var col = (startColor & 0x00FFFFFF) | (alpha << 24);
			var sz = p.size * (p.life / p.maxLife) * scale;
			ImGui.ImDrawList_AddCircleFilled(dl,
				ImGui.vec2(originX + p.x * scale, originY + p.y * scale), sz, col, 6);
		}
	}

	function drawBeam(dl:Dynamic, drawX:Float, drawY:Float, radius:Float, color:Int, scale:Float):Void {
		var angle = progress * Math.PI * 2.0;
		var endX = drawX + Math.cos(angle) * radius * 2.0;
		var endY = drawY + Math.sin(angle) * radius * 2.0;
		var alpha = Std.int(255 * (1.0 - progress));
		var col = (color & 0x00FFFFFF) | (alpha << 24);
		ImGui.ImDrawList_AddLine(dl, ImGui.vec2(drawX, drawY), ImGui.vec2(endX, endY), col, thickness * 2.0 * scale);
	}
