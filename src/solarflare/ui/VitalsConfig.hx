...
/**
 * F6 → Vitals. Showing toggles, chrome, size, HP/Rage/Mana visual styles.
...
class VitalsConfig {
	...
	public static inline var HP_MIN_H:Single = 38;
	...
	public static function compactRow(style:Int, vertical:Bool):Bool {
		return CompactHudLayout.horizontalRow(style, vertical);
	}
	...
	public function draw():Void {
		if (!open.get())
			return;
		var initialSize:ImVec2 = ImGui.vec2(540, 0);
		ImGui.setNextWindowSize(initialSize, ImGuiCond.FirstUseEver);
		if (HudChrome.beginPanel("Vitals", open, "Vitals")) {
			ImGui.textWrapped("HP / Rage / Spark·Mana / Prayers overlay. Thesd uses inward arcs so Health and Rage/Spark can flank the hero.");

			UiLayout.propertyGrid("##vitals_properties", function() {
				UiLayout.propertyRow("Arrangement", function() {
					drawLayoutCombo();
				});

				UiLayout.propertyRow("Health", function() {
					drawVisibilityPair("hp", hpHidden, hpVertical);
				});
				UiLayout.propertyRow("Health style", function() {
					drawStyleCombo("##vitals_hp_style", hpStyle, true, true);
				});

				UiLayout.propertyRow("Rage", function() {
					drawVisibilityPair("rage", rageHidden, rageVertical);
				});
				UiLayout.propertyRow("Rage style", function() {
					drawStyleCombo("##vitals_rage_style", rageStyle, true, false);
				});

				var resourceName = HealthCache.sparkValid ? "Spark" : "Mana";
				UiLayout.propertyRow(resourceName, function() {
					drawVisibilityPair("mana", manaHidden, manaVertical);
				});
				UiLayout.propertyRow(resourceName + " style", function() {
					drawStyleCombo("##vitals_mana_style", manaStyle, true, !HealthCache.sparkValid);
				});

				UiLayout.propertyRow("Prayers", function() {
					if (UiChrome.showingCheckbox("Showing##prayers", prayersHidden))
						SettingsStore.markDirty();
				});

				UiLayout.propertyRow("Health window", function() {
					UiLayout.inlinePair(
						"##vitals_window_toggles",
						function(_:Single) {
							if (ImGui.checkbox("Lock##vitals", chrome.locked))
								SettingsStore.markDirty();
						},
						function(_:Single) {
							if (ImGui.checkbox("Transparent##vitals", chrome.transparent))
								SettingsStore.markDirty();
						}
					);
				});

				UiLayout.propertyRow("Health size", function() {
					UiLayout.inlinePair(
						"##vitals_health_size",
						function(_:Single) {
							if (ImGui.sliderFloat("##vitals_width", hpWidth, HP_MIN_W, HP_MAX_W, "Width %.0f px")) {
								hpSizeDirty = true;
								SettingsStore.markDirty();
							}
						},
						function(_:Single) {
							if (ImGui.sliderFloat("##vitals_height", hpHeight, HP_MIN_H, HP_MAX_H, "Height %.0f px")) {
								hpSizeDirty = true;
								SettingsStore.markDirty();
							}
						}
	...
	static function drawVisibilityPair(id:String, hidden:BoolRef, vertical:BoolRef):Void {
		UiLayout.inlinePair(
			"##vitals_" + id + "_visibility",
			function(_:Single) {
				if (UiChrome.showingCheckbox("Showing##" + id, hidden))
					SettingsStore.markDirty();
			},
			function(_:Single) {
				if (ImGui.checkbox("Vertical stack##" + id + "_vert", vertical))
					SettingsStore.markDirty();
			}
		);
	}
	...
	public function drawLayoutCombo():Void {
		...
		if (ImGui.beginCombo("##vitals_arrangement", preview)) {
...
class RingGauge {
	public static function draw(dl:Dynamic, center:imgui.Structs.ImVec2, radius:Single, ratio:Float,
		...
		var rimCol = ImGui.colorConvertFloat4ToU32(ImGui.vec4(
			Math.min(1, color.x * 1.15 + 0.08),
			Math.min(1, color.y * 1.1 + 0.06),
			Math.min(1, color.z * 1.05 + 0.04), 0.55 * color.w));

		// Soft outer halo for depth (not a thin 1px arc).
		ImGui.ImDrawList_AddCircle(dl, center, radius + thickness * 0.35, glowCol, 48, thickness * 0.55);
		ImGui.ImDrawList_PathClear(dl);
		ImGui.ImDrawList_PathArcTo(dl, center, radius, start, end, 56);
		ImGui.ImDrawList_PathStroke(dl, trackCol, thickness, 0);
		ImGui.ImDrawList_AddCircle(dl, center, radius - thickness * 0.55, rimCol, 48, 1.4 * vectorScale);
		if (ratio > 0.001) {
			ImGui.ImDrawList_PathClear(dl);
			ImGui.ImDrawList_PathArcTo(dl, center, radius, start, fillEnd, 56);
			ImGui.ImDrawList_PathStroke(dl, fillCol, thickness, 0);
			if (ratio < 0.995) {
				var tipX:Single = center.x + Math.cos(fillEnd) * radius;
				var tipY:Single = center.y + Math.sin(fillEnd) * radius;
				ImGui.ImDrawList_AddCircleFilled(dl, ImGui.vec2(tipX, tipY),
					Math.max(2.5 * vectorScale, thickness * 0.42), fillCol, 16);
			}
		}
		if (label != null && label.length > 0) {
			var ts = ImGui.calcTextSize(label);
