...
class ThemePalette {
	...
	public static function drawThemeEditorPane():Void {
		init();
		UiChrome.centeredHeader("Theme");
		ImGui.spacing();
		ImGui.textWrapped("Pick a preset card or refine Custom channels. Theme settings persist in your universal F6 profile.");
		ImGui.spacing();

		var currentTheme = SettingsStore.currentTheme();
		var cols = UiLayout.columnCount(ImGui.getContentRegionAvail().x, 150, 4, 6);
		if (cols < 1) cols = 1;
		var i = 0;
		while (i < presets.length) {
			var rowEnd = Std.int(Math.min(presets.length, i + cols));
			var rowCount = rowEnd - i;
			UiLayout.inlineSplit("##th_preset_row_" + i, rowCount, function(index:Int, width:Single) {
				var p = presets[i + index];
				if (UiChrome.navButton(themeLabel(p) + "##th_p_" + p,
						currentTheme == p, ImGui.vec2(width, 34))) {
					if (SettingsStore.currentTheme() != p) {
		...
		ImGui.spacing();
		UiLayout.propertyGrid("##th_opacity_grid", function() {
			UiLayout.propertyRow("Window opacity", function() {
				if (ImGui.sliderFloat("##th_opacity", windowOpacity, 0.2, 1.0, "%.2f"))
					SettingsStore.markDirty();
			});
		});
		...
		if (SettingsStore.currentTheme() == ThemeKind.Custom) {
			ImGui.spacing();
			UiChrome.subHeader("Custom Color Channels");
			ImGui.spacing();
			UiLayout.propertyGrid("##th_custom_colors", function() {
				UiLayout.propertyRow("Window BG", function() {
					if (ImGui.colorEdit4("##th_winbg", customBufWindowBg)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Control / Frame", function() {
					if (ImGui.colorEdit4("##th_cellbg", customBufCellBg)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Title Bar", function() {
					if (ImGui.colorEdit4("##th_titlebg", customBufTitleBg)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Border", function() {
					if (ImGui.colorEdit4("##th_border", customBufBorder)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Accent", function() {
					if (ImGui.colorEdit4("##th_accent", customBufAccent)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Text", function() {
					if (ImGui.colorEdit4("##th_text", customBufText)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Muted Text", function() {
					if (ImGui.colorEdit4("##th_textdis", customBufTextDisabled)) SettingsStore.markDirty();
				});
				UiLayout.propertyRow("Dim Overlay", function() {
					if (ImGui.colorEdit4("##th_dim", customBufDimOverlay)) SettingsStore.markDirty();
				});
			});

			ImGui.spacing();
			UiLayout.inlinePair("##th_copy_presets", function(w:Single) {
				if (UiChrome.ghostButton("Copy Purple & Gold##th_cpy_pg", ImGui.vec2(w, 30))) {
					copyPresetToCustom(ThemeKind.PurpleGold);
					SettingsStore.markDirty();
				}
			}, function(w:Single) {
				if (UiChrome.ghostButton("Copy Obsidian Ember##th_cpy_oe", ImGui.vec2(w, 30))) {
					copyPresetToCustom(ThemeKind.ObsidianEmber);
					SettingsStore.markDirty();
				}
			}, 8);
		}
