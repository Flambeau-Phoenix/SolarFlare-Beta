...
class CombatLogCache {
	...
	static function fillTargetHp(unit:Dynamic):Void {
		if (unit == null)
			return;
		targetSnap.targetLevelValid = false;
		targetSnap.playerLevelValid = false;
		targetSnap.deadKnown = false;
		targetSnap.dead = false;
		try {
			var u:ent.Unit = cast unit;
			targetSnap.targetLevel = u.get_level();
			targetSnap.targetLevelValid = targetSnap.targetLevel > 0;
		} catch (_:Dynamic) {}
		try {
			var go:ent.GameObject = cast unit;
			targetSnap.dead = go.isDead();
			targetSnap.deadKnown = true;
		} catch (_:Dynamic) {}
		try {
			var player:ent.Unit = cast solarflare.HealthCache.localHero;
			if (player != null) {
				targetSnap.playerLevel = player.get_level();
				targetSnap.playerLevelValid = targetSnap.playerLevel > 0;
			}
		} catch (_:Dynamic) {}
		var hp = unitHealth(unit);
		targetSnap.healthValid = Math.isFinite(hp.cur) && Math.isFinite(hp.max) && hp.max > 0 && hp.cur >= 0;
		if (!targetSnap.healthValid) {
			if (!targetSnap.valid)
				return;
			targetSnap.health = 0;
