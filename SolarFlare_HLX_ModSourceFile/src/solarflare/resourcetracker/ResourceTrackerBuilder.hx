...
class ResourceTrackerBuilder {
	...
	public var selected:String = "health";
	...
	public function draw():Void {
		...
		var vis = ToolWindow.begin("Resource Tracker Builder###SolarFlare.ResourceTracker", open, 980, 700);
		if (vis) {
			ImGui.textWrapped("Configure each resource window and preview deterministic combat states without moving the live HUD.");
			ImGui.separator();
			HudChrome.safeChild("##rt_builder_left", ImGui.vec2(350, 0), 0, drawNavigation);
			ImGui.sameLine();
			HudChrome.safeChild("##rt_builder_right", ImGui.vec2(0, 0), 0, function() {
				drawPreview();
				ImGui.separatorText("Settings — " + selectedLabel());
	...
	function drawNavigation():Void {
		...
		ImGui.separator();
		ImGui.separatorText("Core Resources");
		var v = host.vitals;
		if (v != null) {
			navRow("health", "Health", v.hpHidden, v.chrome);
		...
		ImGui.separatorText("Combat Resources");
		if (host.combo != null) navRow("combo", "Combo Points", host.combo.hidden, host.combo.chrome);
		if (host.attackCombo != null) navRow("attack", "Attack Combo", host.attackCombo.hidden, host.attackCombo.chrome);
	...
	function navRow(id:String, label:String, hidden:BoolRef, chrome:HudChrome):Void {
		...
		ImGui.sameLine();
		if (ImGui.smallButton((selected == id ? "Editing" : "Select/Edit") + "##rt_nav_edit_" + id)) selected = id;
		ImGui.separator();
	}

	function drawPreview():Void {
		ImGui.separatorText("Preview");
		if (ImGui.beginCombo("Mode##rt_preview_scope", previewScope.get() == 1 ? "All elements" : "Selected element")) {
		...
		ImGui.sameLine();
		if (ImGui.beginCombo("State##rt_preview_state", scenarioLabel(previewScenario.get()))) {
	...
	function drawSelectedSettings():Void {
		...
		switch (selected) {
			...
			case "prayers":
				...
				ImGui.separatorText("Appearance"); ImGui.textDisabled("Prayer readiness uses the three semantic prayer colors.");
			...
			case "attack":
				...
				ImGui.separatorText("Appearance"); host.attackCombo.drawAppearanceSettings();
				ImGui.separatorText("Behavior"); host.attackCombo.drawBehaviorSettings();
			case "target":
				if (host.target != null)
			...
			case "chaincast":
				...
				ImGui.separatorText("Appearance"); ImGui.textDisabled("Chaincast uses its fixed rail and ready-gem presentation.");
				ImGui.separatorText("Behavior"); host.chaincast.drawSettings();
			case "conduit":
	...
	function drawBehaviorText(text:String):Void { ImGui.separatorText("Behavior"); ImGui.textWrapped(text); }
	...
	function selectedLabel():String return switch (selected) {
		...
		case "combo": "Combo Points"; case "attack": "Attack Combo"; case "target": "Current Target";
