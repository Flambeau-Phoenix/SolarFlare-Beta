...
class DragDropHelper {
	...
	/**
	 * Drop target for aura slots with item hover detection.
	 */
	public static function auraSlotDropTarget(slotIndex:Int, currentAuraId:Int):Int {
		var result = -1;
		if (ImGui.isItemHovered() && ImGui.beginDragDropTarget()) {
			var payload = ImGui.acceptDragDropPayload(PAYLOAD_AURA);
			if (payload != null && ImGui.ImGuiPayload_IsDataType(payload, PAYLOAD_AURA)) {
				result = PayloadExt.getInt(payload);
			}
			ImGui.endDragDropTarget();
		}
		return result;
	...
	/**
	 * Begin dragging a generic text payload.
	 */
	public static function beginTextDrag(text:String, displayText:String = null):Bool {
		if (!ImGui.beginDragDropSource(0))
			return false;

		var idBytes = haxe.io.Bytes.ofString(text);
		ImGui.setDragDropPayload(PAYLOAD_TEXT, idBytes.getData(), idBytes.length + 1);
	...
	/**
	 * Accept a text payload.
	 */
	public static function acceptTextDrop():String {
		if (!ImGui.beginDragDropTarget())
			return null;

		var result:String = null;
		var payload = ImGui.acceptDragDropPayload(PAYLOAD_TEXT);
		if (payload != null && ImGui.ImGuiPayload_IsDataType(payload, PAYLOAD_TEXT)) {
			result = PayloadExt.getString(payload);
		}
