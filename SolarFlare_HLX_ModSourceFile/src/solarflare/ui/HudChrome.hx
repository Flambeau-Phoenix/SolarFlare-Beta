 ...
 * Sun grip (click collapses, hold-drag moves), X closes. Lock disables
 * interaction without changing strip geometry. Transparent drops the strip
 * art but keeps the grip live, so a fully invisible window still moves while
 * unlocked. The strip also covers the top resize border, which is what keeps
 * blit assets from being grabbed in short HUD windows.
 */
class HudChrome {
	...
	/** Runtime widgets persist content-anchor migration independently of legacy tool chrome. */
	public var hudLayoutVersion:Int = 0;
	...
	/** Child flags for the body scope (HUD widgets pad so the strip cannot clip their art). */
	var bodyChildFlags:Int = 0;
	/** True only when the matching ImGui.begin() for this panel succeeded. */
	var windowBegan:Bool = false;
	/** Set by beginPanel: editors use the native F6 bar, so beginBody skips the strip. */
	var nativeTitle:Bool = false;
	var stylePushed:Bool = false;
	...
	public static function beginPanel(name:String, ?open:BoolRef, caption:String = "", extraFlags:Int = 0, ?chrome:HudChrome,
		...
		c.stylePushed = true;
		// `###name` keeps the original ImGui ID while the bar shows the caption.
		var label = (caption != null && caption.length > 0) ? caption + "###" + name : name;
		var shown = ImGui.begin(label, open, flags);
		c.windowBegan = true;
		endStack.push(c);
		if (!shown)
			return false;
	...
	public function drawToggles(id:String):Bool {
		var changed = false;
		var chip = ImGui.vec2(0, 28);
		if (UiChrome.toggleChip("Lock##" + id, locked, chip))
			changed = true;
		ImGui.sameLine(0, 6);
		if (UiChrome.toggleChip("Transparent##" + id, transparent, chip))
			changed = true;
		if (changed)
			SettingsStore.markDirty();
		return changed;
	...
	public function drawWindowSettings(hidden:BoolRef, id:String):Bool {
		var changed = false;
		UiLayout.propertyGrid("##" + id + "_window_settings", function() {
			if (hidden != null) {
				UiLayout.propertyRow("Showing", function() {
					if (UiChrome.showingTile("##" + id + "_show", "Showing", hidden, 108, 28))
						changed = true;
				});
			}
			UiLayout.propertyRow("Window", function() {
				UiLayout.inlinePair(
					"##" + id + "_lock_trans",
					function(w:Single) {
						if (UiChrome.toggleTileRef("##" + id + "_lock", "Lock", locked, w, 28))
							changed = true;
					},
					function(w:Single) {
						if (UiChrome.toggleTileRef("##" + id + "_trans", "Transparent", transparent, w, 28))
							changed = true;
					}
				);
			});
	...
	/**
	 * Thin title: grip (collapse / drag) + optional leading + caption + X.
	 * Art is skipped while transparent, but the grip stays live so the window
	 * can still be moved. Returns false when collapsed or just closed.
	 */
	public function drawTitleTools(?onClose:Void->Void, ?drawLeading:Void->Void, ?caption:String):Bool {
		...
		if (!isTransparent()) {
			...
			ImGui.ImDrawList_AddLine(dl, ImGui.vec2(wp.x + 4, wp.y + STRIP + 1),
				ImGui.vec2(wp.x + ws.x - 4, wp.y + STRIP + 1), accentCol, 1);
		}
		...
		// Transparent hides the X with the rest of the strip art. The grip stays live
		// so the window still moves, but an invisible close button would be a trap.
		var closeHit = false;
		if (onClose != null && !isTransparent()) {
	...
	/** Owner-supplied context-menu entries (Open Builder, Hide, per-widget actions). */
	public var extraMenu:Void->Void = null;
	...
	public function beginBody(?onClose:Void->Void, ?drawLeading:Void->Void, ?caption:String, scrollBody:Bool = false, bodyFlags:Int = 0,
		...
		if (nativeTitle) {
			// Editors already have the native bar; only the body child is ours.
			if (collapsed.get())
				return false;
			if (drawLeading != null)
				drawLeading();
	...
	function blitSun(px:Single, py:Single):Void {
		...
		if (!GameIcons.draw(dl, tex, px, py, SUN)) {
			...
			ImGui.ImDrawList_AddCircleFilled(dl, ImGui.vec2(cx, cy), 4.2, col, 16);
			for (i in 0...8) {
				var a = i * Math.PI / 4;
				ImGui.ImDrawList_AddLine(dl,
					ImGui.vec2(cx + Math.cos(a) * 5.4, cy + Math.sin(a) * 5.4),
					ImGui.vec2(cx + Math.cos(a) * 8.2, cy + Math.sin(a) * 8.2), col, 1.7);
			}
		}
	}
	...
	function drawCloseGlyph(px:Single, py:Single):Void {
		...
		ImGui.ImDrawList_AddLine(dl, ImGui.vec2(px + pad, py + pad),
			ImGui.vec2(px + CLOSE - pad, py + CLOSE - pad), col, 1.6);
		ImGui.ImDrawList_AddLine(dl, ImGui.vec2(px + CLOSE - pad, py + pad),
			ImGui.vec2(px + pad, py + CLOSE - pad), col, 1.6);
	}
