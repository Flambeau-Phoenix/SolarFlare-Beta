...
class HealthCache {
	...
	public static var deadKnown:Bool = false;
	...
	/**
	 * Hot path: called up to six times per status / skill hook, for every entity in the
	 * zone. The pointer compare is free but a miss used to pay a native isMyHero() every
	 * time, so misses are memoized in a small ring.
	 *
	 * The ring is bounded three ways because a false negative would strand the HUD: it
	 * holds few entries, it is dropped on any identityGen bump, and it expires on a short
	 * TTL so a hero the engine has not yet flagged as mine is always retried.
	 */
	...
	static inline var MISS_RING:Int = 8;
	static inline var MISS_TTL_S:Float = 0.5;
	static var missRing:Array<Dynamic> = [];
	static var missAt:Int = 0;
	static var missGen:Int = -1;
	static var missUntil:Float = 0;

	static function missKnown(unit:Dynamic):Bool {
		if (missGen != identityGen || nowSec() >= missUntil) {
			resetMissRing();
			return false;
		}
		var i = 0;
		while (i < missRing.length) {
			if ((cast missRing[i] : Dynamic) == (cast unit : Dynamic))
				return true;
			i++;
		}
		return false;
	...
	static function rememberMiss(unit:Dynamic):Void {
		if (missRing.length == 0)
			missUntil = nowSec() + MISS_TTL_S;
		missGen = identityGen;
		if (missRing.length < MISS_RING) {
			missRing.push(unit);
			return;
		}
		missRing[missAt] = unit;
		missAt = (missAt + 1) % MISS_RING;
	}

	/** Release memoized pointers so a dead stage's entity graph can be collected. */
	static function resetMissRing():Void {
		missGen = identityGen;
		missAt = 0;
		missUntil = nowSec() + MISS_TTL_S;
		if (missRing.length > 0)
			missRing.splice(0, missRing.length);
	}
