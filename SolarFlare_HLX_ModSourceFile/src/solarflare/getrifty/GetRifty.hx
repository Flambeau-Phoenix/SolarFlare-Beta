...
class GetRiftyConfig {
	...
	public function draw():Void {
		...
		if (HudChrome.beginPanel("GetRifty##solarflare_cfg", open, "GetRifty")) {
			...
			if (ImGui.beginCombo("Clock style##rifty", preview)) {
			...
			if (ImGui.button("Test portal (:45)"))
				GetRiftyCache.forceAlert(GetRiftyCache.ALERT_PORTAL);
			ImGui.sameLine();
			if (ImGui.button("Test rift (:00)"))
				GetRiftyCache.forceAlert(GetRiftyCache.ALERT_RIFT);
			if (ImGui.collapsingHeader("ASCII / text hole")) {
				if (ImGui.sliderFloat("Text left##rifty_u0", textU0, 0, 1, "%.2f"))
					SettingsStore.markDirty();
				if (ImGui.sliderFloat("Text top##rifty_v0", textV0, 0, 1, "%.2f"))
					SettingsStore.markDirty();
				if (ImGui.sliderFloat("Text right##rifty_u1", textU1, 0, 1, "%.2f"))
					SettingsStore.markDirty();
				if (ImGui.sliderFloat("Text bottom##rifty_v1", textV1, 0, 1, "%.2f"))
					SettingsStore.markDirty();
				...
				ImGui.text("Timer draws in this region of the art.");
				drawCropPreview();
			}
		}
		HudChrome.endPanel();
...
class GetRiftyOverlay {
	...
	function drawWindow(cfg:GetRiftyConfig):Void {
		...
		ImGui.setNextWindowSizeConstraints(ImGui.vec2(GetRiftyConfig.MIN_SIZE, GetRiftyConfig.MIN_SIZE),
			ImGui.vec2(GetRiftyConfig.MAX_SIZE, GetRiftyConfig.MAX_SIZE + 60));
		if (cfg.sizeDirty) {
			ImGui.setNextWindowSize(ImGui.vec2(winW, winH), ImGuiCond.Always);
			cfg.sizeDirty = false;
		} else {
			ImGui.setNextWindowSize(ImGui.vec2(winW, winH), ImGuiCond.FirstUseEver);
