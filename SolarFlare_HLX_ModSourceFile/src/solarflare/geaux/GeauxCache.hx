...
class GeauxCache {
	...
	static function fillEmptyFromBar(visibleCount:Int):Void {
		var used = new Map<String, Bool>();
		for (i in 0...visibleCount) {
			if (slots[i].present && slots[i].id.length > 0)
				used.set(slots[i].id, true);
		}
		var bi = 0;
		for (i in 0...visibleCount) {
			if (slots[i].present)
				continue;
			while (bi < weapons.length && (!weapons[bi].present || weapons[bi].id.length == 0 || used.exists(weapons[bi].id)))
				bi++;
			if (bi >= weapons.length)
				return;
			copySnap(slots[i], weapons[bi], i);
			used.set(weapons[bi].id, true);
			bi++;
		}
	}
	...
	static function fillEmptyWithClassSkills(visibleCount:Int):Void {
		var used = new Map<String, Bool>();
		for (i in 0...visibleCount) {
			if (slots[i].present && slots[i].id.length > 0)
				used.set(slots[i].id, true);
		...
		for (i in 0...visibleCount) {
			if (slots[i].present)
				continue;
			while (ci < classSkills.length
				&& (!classSkills[ci].present || classSkills[ci].id.length == 0 || used.exists(classSkills[ci].id)))
				ci++;
			if (ci >= classSkills.length)
				return;
			copySnap(slots[i], classSkills[ci], i);
			used.set(classSkills[ci].id, true);
			ci++;
		}
	}

	static function fillEmptyWithSignatures(visibleCount:Int):Void {
		var used = new Map<String, Bool>();
		for (i in 0...visibleCount) {
			if (slots[i].present && slots[i].id.length > 0)
				used.set(slots[i].id, true);
		...
		for (i in 0...visibleCount) {
			...
			while (si < signatures.length
				&& (!signatures[si].present
					|| signatures[si].id.length == 0
					|| used.exists(signatures[si].id)
					|| !signatureOwnedByHero(signatures[si])))
				si++;
			if (si >= signatures.length)
				return;
			copySnap(slots[i], signatures[si], i);
			used.set(signatures[si].id, true);
			si++;
		}
	}

	static function signatureOwnedByHero(snap:GeauxSlotSnap):Bool {
		if (snap == null || snap.id == null || snap.id.length == 0)
			return false;
		if (solarflare.PrayerCache.isJudgementId(snap.id))
			return solarflare.PrayerCache.active;
		if (cachedSkill(snap.id) != null)
			return true;
		if (snap.iconId != null && snap.iconId.length > 0 && cachedSkill(snap.iconId) != null)
			return true;
		for (s in weapons) {
			if (s == null || !s.present)
				continue;
			if (s.id == snap.id || s.iconId == snap.id)
				return true;
			if (snap.iconId.length > 0 && (s.id == snap.iconId || s.iconId == snap.iconId))
				return true;
		}
		return false;
	}

	static function fillEmptyWithPrayers(visibleCount:Int):Void {
		if (!solarflare.PrayerCache.active)
			return;
		var used = new Map<String, Bool>();
		for (i in 0...visibleCount) {
		...
		var prayers = [
			solarflare.PrayerCache.smiteId(),
			solarflare.PrayerCache.lifeId(),
			solarflare.PrayerCache.shieldId()
		];
		var pi = 0;
		for (i in 0...visibleCount) {
			if (slots[i].present)
				continue;
			while (pi < prayers.length && (prayers[pi] == null || prayers[pi].length == 0 || used.exists(prayers[pi])))
				pi++;
			if (pi >= prayers.length)
				return;
			fillPrayerSnap(slots[i], prayers[pi], i);
			used.set(prayers[pi], true);
			pi++;
		}
	}
	...
	/**
	 * Mirror vanilla action-bar widgets. Class/arsenal live on SkillSlotButton /
	 * WeaponSkillSlotButton wrappers: ids + getOverrideSkill on the parent, pinwheel
	 * + cdTimer on the inner SkillButton.
	 */
	static function applyHudCooldowns(hero:Dynamic):Void {
		var byId = new Map<String, {left:Float, max:Float, remaining:Float}>();
		var byIndex = new Map<Int, {left:Float, max:Float, remaining:Float}>();
		hudWalkNodes = 0;
		hudHero = hero;
		try {
			var hud:Dynamic = null;
			...
			var bottom = plainField(hud, "bottomBar");
			walkHudNode(plainField(bottom, "heroBar"), byId, byIndex, 0);
			walkHudNode(plainField(bottom, "heroBarPad"), byId, byIndex, 0);
			walkHudNode(plainField(bottom, "skillBar"), byId, byIndex, 0);
			walkHudNode(bottom, byId, byIndex, 0);
			walkHudNode(plainField(hud, "widgets"), byId, byIndex, 0);
			walkHudNode(plainField(hud, "skillBar"), byId, byIndex, 0);
		} catch (_:Dynamic) {}
		hudHero = null;
		...
		for (i in 0...count) {
			...
			if (!snap.present || snap.id == null || snap.id.length == 0)
				continue;
			var cd = byId.exists(snap.id) ? byId.get(snap.id) : null;
			if (cd == null) {
			...
			if (cd == null && byIndex.exists(i) && (snap.group == "" || snap.group == "BAR" || snap.group == "WEP" || snap.group == "CLS" || snap.group == "SIG"))
				cd = byIndex.get(i);
			if (cd == null)
				continue;
			if (cd.left <= 0.05 && cd.remaining <= 0.02)
				continue;
			// HUD never raises remaining (vanilla bar can lag behind talent CDR).
			// Adopt when snap has no CD yet, or HUD reports a lower remaining.
			if (snap.cdLeft > 0.05 && cd.left >= snap.cdLeft - 0.02)
				continue;
			snap.cdLeft = cd.left;
			if (cd.max > snap.cdMax)
				snap.cdMax = cd.max;
			if (cd.remaining > 0.02)
				snap.remaining = cd.remaining;
			else if (snap.cdMax > 0.05)
				snap.remaining = clamp01(snap.cdLeft / snap.cdMax);
			snap.ready = snap.cdLeft <= 0.05 && snap.remaining <= 0.02;
		}
	}

	static function applyInputCooldowns(hero:Dynamic):Void {
		if (hero == null)
			return;
		if (byInputMem == null)
			byInputMem = resolveMem("ent.Hero", "getSkillByInput");
		...
		var names = [
			"Skill1", "Skill2", "Skill3", "Skill4", "Skill5", "Skill6", "Skill7", "Skill8",
			"ClassSkill1", "ClassSkill2", "ClassSkill3", "ClassSkill4", "ClassSkill5", "ClassSkill6",
			"ArsenalSkill1", "ArsenalSkill2", "ArsenalSkill3", "ArsenalSkill4",
			"WeaponSkill1", "WeaponSkill2", "OffhandSkill", "OffhandSkill1",
			"SignatureSkill", "Signature", "SigSkill", "ClassSignature", "Ultimate", "UltimateSkill"
		];
		var live = new Map<String, Dynamic>();
		for (inp in names) {
			var skill = callResolved(byInputMem, [hero, inp]);
			if (skill == null)
				continue;
			var parsed = parseSkillish(skill);
			if (parsed.id.length > 0)
				live.set(parsed.id, parsed.skill != null ? parsed.skill : skill);
		}
		for (i in 0...count) {
			var snap = slots[i];
			if (!snap.present || snap.id.length == 0)
				continue;
			if (snap.cdLeft > 0.05)
				continue;
			if (!live.exists(snap.id))
				continue;
			applyCooldown(hero, snap, live.get(snap.id));
		}
	}

	static function walkHudNode(node:Dynamic, byId:Map<String, {left:Float, max:Float, remaining:Float}>, byIndex:Map<Int, {left:Float, max:Float, remaining:Float}>, depth:Int):Void {
		if (node == null || depth > 18 || hudWalkNodes > 1800)
			return;
		hudWalkNodes++;
		ingestHudSlot(node, byId, byIndex);
		var follow = [
			"button", "skillBar", "skillBarPad", "primaryWeaponBar", "weapons",
			"secondaryWeaponBar", "offhandWeaponBar", "arsenalWeaponBar", "extraWeaponBar",
			"weaponBar", "classSkills", "skills"
		...
		for (name in follow)
			walkHudNode(plainField(node, name), byId, byIndex, depth + 1);
		var arrNames = ["skillSlots", "slots", "buttons", "skillButtons", "weaponBars"];
		for (name in arrNames) {
			...
			if (n <= 0 || n > 48)
				continue;
			for (i in 0...n)
				walkHudNode(arrayAt(arr, i), byId, byIndex, depth + 1);
		}
		var kids = objectChildren(node);
		var kn = arrayLen(kids);
		for (i in 0...kn)
			walkHudNode(arrayAt(kids, i), byId, byIndex, depth + 1);
	}

	static function ingestHudSlot(node:Dynamic, byId:Map<String, {left:Float, max:Float, remaining:Float}>, byIndex:Map<Int, {left:Float, max:Float, remaining:Float}>):Void {
		try {
			var typedBtn:ui.hud.SkillButton = node;
			if (typedBtn != null)
				ingestTypedButton(typedBtn, byId, byIndex, -1);
		} catch (_:Dynamic) {}
		try {
			var slot:ui.hud.SkillSlotButton = node;
			if (slot != null && slot.button != null)
				ingestTypedButton(slot.button, byId, byIndex, slot.index);
		} catch (_:Dynamic) {}
		try {
			var wslot:ui.hud.WeaponSkillSlotButton = node;
			if (wslot != null && wslot.button != null)
				ingestTypedButton(wslot.button, byId, byIndex, -1);
		} catch (_:Dynamic) {}
		var btn = node;
		if (plainField(node, "cdTimer") == null)
			btn = plainField(node, "button");
		if (btn == null)
			btn = node;
		var hasTimer = plainField(btn, "cdTimer") != null || plainField(btn, "cooldown") != null;
		var hasSlot = plainField(node, "index") != null || plainField(node, "input") != null || plainField(node, "button") != null;
		if (!hasTimer && !hasSlot)
			return;

		var skill = plainField(btn, "skill");
		if (skill == null)
			skill = plainField(node, "skill");
		var over = callOverrideSkill(node);
		if (over == null)
			over = callOverrideSkill(btn);
		if (skill == null)
			skill = over;
		else if (over != null)
			skill = over;
		...
		var input = dynString(plainField(node, "input"));
		if (input.length == 0)
			input = dynString(plainField(btn, "input"));
		if ((skill == null || parseSkillish(skill).id.length == 0) && input.length > 0 && hudHero != null)
			skill = callResolved(byInputMem, [hudHero, input]);

		var ids:Array<String> = [];
		addSkillId(ids, skillIdFromButton(btn));
		addSkillId(ids, skillIdFromButton(node));
		addSkillId(ids, parseSkillish(skill).id);
		addSkillId(ids, parseSkillish(over).id);
		if (input.length > 0 && hudHero != null)
			addSkillId(ids, parseSkillish(callResolved(byInputMem, [hudHero, input])).id);
		if (ids.length == 0 && !hasTimer)
			return;
		pinBarEntry(ids, skill, btn);

		var max = readMaxCd(skill);
		if (max <= 0)
			max = typedMaxCd(skill);
		var left = parseCdText(textOf(plainField(btn, "cdTimer")));
		if (Math.isNaN(left))
			left = parseCdText(textOf(plainField(btn, "serverCdTimer")));
		if (Math.isNaN(left) || left < 0) {
			left = typedLeftCd(skill, max);
			if (Math.isNaN(left) || left < 0) {
				if (cdLeftMem == null)
					cdLeftMem = resolveMem("st.skill.Skill", "getCooldownLeft");
				left = callNamedFloat(skill, "getCooldownLeft", cdLeftMem);
		...
		var radial = plainField(btn, "cooldown");
		var prog = FieldWalk.extractNumber(radial, "progress", Math.NaN);
		if (Math.isNaN(prog) || prog <= 0)
			prog = typedProgress(skill);
		var remaining = 0.0;
		if (left <= 0.05 && !Math.isNaN(prog) && prog > 0.05 && prog <= 1.0001) {
			remaining = clamp01(prog);
			left = max > 0.05 ? remaining * max : 1;
		} else if (left > 0.05) {
			if (!Math.isNaN(prog) && prog > 0.05 && prog <= 1.0001)
				remaining = clamp01(prog);
			else if (max > 0.05)
				remaining = clamp01(left / max);
			else
				remaining = 1;
		}
		var cd = {left: left, max: max > 0 ? max : left, remaining: remaining};
		for (id in ids)
			putHudCd(byId, id, cd);

		var idx = -1;
		try {
			var iv = plainField(node, "index");
			if (iv != null)
				idx = Std.int(iv);
		} catch (_:Dynamic) {}
		if (idx >= 0 && (hasTimer || ids.length > 0))
			byIndex.set(idx, cd);
	}

	static function ingestTypedButton(btn:ui.hud.SkillButton, byId:Map<String, {left:Float, max:Float, remaining:Float}>, byIndex:Map<Int, {left:Float, max:Float, remaining:Float}>, indexHint:Int):Void {
		if (btn == null)
			return;
		var id = "";
		try {
			if (btn.inf != null && btn.inf.id != null)
				id = btn.inf.id;
		} catch (_:Dynamic) {}
		...
		if ((id == null || id.length == 0) && skill != null)
			id = getSkillId(skill);
		if (id != null && id.length > 0)
			pinBarEntry([id], skill, btn);
		var max = 0.0;
		var left = Math.NaN;
		try {
			var t = btn.cdTimer;
			if (t != null)
				left = parseCdText(t.text);
		} catch (_:Dynamic) {}
		try {
			if (Math.isNaN(left) || left < 0) {
				var t = btn.serverCdTimer;
				if (t != null)
					left = parseCdText(t.text);
			}
		} catch (_:Dynamic) {}
		if (skill != null) {
			try
				max = skill.getEffectiveCooldown()
			catch (_:Dynamic) {}
			if (max <= 0) {
				try
					max = skill.getCooldown()
				catch (_:Dynamic) {}
			}
			if (max <= 0) {
				try
					max = skill.cooldownDuration
				catch (_:Dynamic) {}
			}
			if (Math.isNaN(left) || left < 0) {
				try
					left = skill.getCooldownLeft()
				catch (_:Dynamic) {}
			}
		}
		...
		var prog = Math.NaN;
		try {
			var radial = btn.cooldown;
			if (radial != null)
				prog = radial.progress;
		} catch (_:Dynamic) {}
		if ((Math.isNaN(prog) || prog <= 0) && skill != null) {
			try
				prog = skill.getCooldownProgress()
			catch (_:Dynamic) {}
		}
		var remaining = 0.0;
		if (left <= 0.05 && !Math.isNaN(prog) && prog > 0.05 && prog <= 1.0001) {
		...
		if (left <= 0.05 && remaining <= 0.02)
			return;
		var cd = {left: left, max: max > 0 ? max : left, remaining: remaining};
		if (id != null && id.length > 0)
			putHudCd(byId, id, cd);
		if (indexHint >= 0)
			byIndex.set(indexHint, cd);
	}

	static function putHudCd(byId:Map<String, {left:Float, max:Float, remaining:Float}>, id:String, cd:{left:Float, max:Float, remaining:Float}):Void {
		if (id == null || id.length == 0)
			return;
		// Prefer the lower remaining — stale widgets must not overwrite talent CDR.
		if (byId.exists(id)) {
			var prev = byId.get(id);
			if (prev.left < cd.left && prev.left > 0.05)
				return;
		}
		byId.set(id, cd);
	}
	...
	static function writeCooldownFromSkill(snap:GeauxSlotSnap, skill:Dynamic):Void {
		if (snap == null || skill == null)
			return;
		pollSkillCooldown(skill, cdPoll);
	...
	static function pollSkillCooldown(skill:Dynamic, out:GeauxCdPoll):Void {
		out.reset();
		var typedOk = false;
		try {
			var typed:st.skill.Skill = skill;
	...
	static function ledgerGeauxCd(method:String, src:String, name:String, hook:String, snap:GeauxSlotSnap):Void {
		...
		if (!snapOnBar(snap))
			return;
		var L = solarflare.debug.ResolutionLedger;
		L.touch("geaux.slot.cdLeft", method, "GeauxCache." + src, name, "number", Std.string(Math.round(snap.cdLeft * 1000) / 1000), "", hook);
		L.touch("geaux.slot.cdMax", method, "GeauxCache." + src, name, "number", Std.string(Math.round(snap.cdMax * 1000) / 1000), "", hook);
		L.touch("geaux.slot.ready", method, "GeauxCache." + src, name, "bool", snap.ready ? "true" : "false", "", hook);
		if (snap.id != null && snap.id.length > 0)
	...
	static function snapOnBar(snap:GeauxSlotSnap):Bool {
		if (snap == null || slots == null)
			return false;
		var i = 0;
		while (i < count) {
			if (slots[i] == snap)
	...
	static function applyTrackedCooldowns():Void {
		var i = 0;
		while (i < count) {
	...
	static function syncTrackedFromLive(id:String, left:Float, max:Float):Void {
		...
		if (left <= 0.05) {
			...
			if (!Math.isNaN(until) && until - nowStamp() > 0.05) {
				if (max > 0.05)
					cdMaxById.set(id, max);
				return;
			}
		...
		var until = nowStamp() + left;
		for (a in skillIdKeys(id)) {
			cdUntil.set(a, until);
			if (max > 0.05)
				cdMaxById.set(a, max);
		}
	...
	static function applyAffordSnap(hero:ent.Hero, snap:GeauxSlotSnap, skill:Dynamic = null):Void {
		...
		if (solarflare.debug.ResolutionLedger.armed() && snapOnBar(snap))
			solarflare.debug.ResolutionLedger.touch("geaux.slot.affordable", "typed", "GeauxCache.applyAffordSnap", "ownerHaveAllCost", "bool", snap.affordable ? "true" : "false");
	}
